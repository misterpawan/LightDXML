...
class ShortlistEnsemble(object):
    ...
    * brute or HNSW algorithm for search
    ...
    def query(self, data, model_dir):
        ...
        #     raise ValueError("Need model directory. Data too large to be in memory.")
        # step_size = 50000
        # indices = np.memmap(f"{model_dir}/query.ind", shape=(len(data), self.efS), mode='w+', dtype='int64')
        # sim = np.memmap(f"{model_dir}/query.val", shape=(len(data), self.efS), mode='w+', dtype='float32')
        # for i in tqdm(range(0, len(data), step_size), desc="Writing distances into file"):
        #     tmp_indices, tmp_distances = self.index.predict(data[i:i+step_size])
        #     indices[i:i+step_size] = tmp_indices[:]
        #     distances[i:i+step_size] = tmp_distances[:]
        # return indices, sim
...
class LabelEmbeddingShortlist(Shortlist):
    """Multiple graphs; Supports parallel training
        Assumes that all parameters are same for each graph
    Parameters
    ----------
    ...
    def __init__(self, num_neighbours, M, efC, efS, num_threads=1, method='hnsw'):
        super(LabelEmbeddingShortlist, self).__init__(method, num_neighbours, M, efC, efS, num_threads, space='cosine')

    def query(self, data, model_dir=None, *args, **kwargs):
        if len(data) < 50000:
        ...
        step_size = 50000
        indices = np.memmap(f"{model_dir}/query.ind", shape=(len(data), self.efS), mode='w+', dtype='int64')
        distances = np.memmap(f"{model_dir}/query.val", shape=(len(data), self.efS), mode='w+', dtype='float32')
        for i in tqdm(range(0, len(data), step_size), desc="Writing distances into file"):
            tmp_indices, tmp_distances = self.index.predict(data[i:i+step_size], *args, **kwargs)
            indices[i:i+step_size] = tmp_indices[:]
            distances[i:i+step_size] = tmp_distances[:]
        return indices, 1 - distances
