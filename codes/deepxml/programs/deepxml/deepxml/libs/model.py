...
from models.label_embedding_layer import LabelEmbed, DistanceInverseProportionalLoss
...
class ModelShortlist(ModelBase):
    ...
    def _predict_shorty(self, doc_embeddings):
        ...
        """
        return self.shorty.query(doc_embeddings, self.model_dir)
    ...
    def _fit(self, train_loader, validation_loader, model_dir, result_dir,
        ...
        for epoch in range(init_epoch, init_epoch+num_epochs):
            ...
            if validation_loader is not None and epoch % validate_after == 0:
                ...
                val_end_t = time.time()
                _acc = self.evaluate(
                    validation_loader.dataset.labels.data, predicted_labels, train_labels=train_loader.dataset.labels.data)
    ...
    def fit(self, data_dir, model_dir, result_dir, dataset, learning_rate,
            ...
            trn_pretrained_shortlist=None, val_pretrained_shortlist=None, label_mapping=None):
        """
...
class ModelLabelEmbedding(ModelBase):
    ...
    def __init__(self, params, net, criterion, optimizer, shorty):
        ...
        self.retrain_hnsw_after = params.retrain_hnsw_after
        self.update_shortlist = params.update_shortlist
        self.label_embed_model = LabelEmbed(params.num_labels, self.embedding_dims, params.label_hidden_dims, device="cuda:1")
        self.label_embed_model = self.label_embed_model.to(self.label_embed_model.device)
        self.num_labels = params.num_labels
        self.num_label_epochs = params.num_label_epochs
        self.encoding_batch_size = params.encoding_batch_size
        self.label_batch_size = params.label_batch_size
        self.label_embed_lr = params.label_embed_lr
    ...
    def _fit_shorty(self):
        ...
        * Features are computed when pre-computed features are not available
        ...
        feature_type: str, optional, default='sparse'
            sparse or dense features
        """
        self.shorty.fit(self.label_embed_model(
            torch.LongTensor(
                list(
                    range(
                        self.num_labels
            ...
            .to(
                 self.label_embed_model.device
            )
        ).detach().cpu().numpy())

    def _train_label_embedding(self, dataset, epoch):
        if isinstance(dataset.features, DenseFeatures):
            self.logger.info("Using pre-trained embeddings for shortlist.")
            doc_embeddings = dataset.features.data
        else:
            doc_embeddings = torch.from_numpy(self.get_embeddings(
                data=dataset.features.data,
                fname_out=None,
                batch_size=self.encoding_batch_size
            )).to(self.label_embed_model.device)

        d = np.split(dataset.labels.data.indices, dataset.labels.data.indptr)[1:-1]
        self.label_embed_model.initialize(doc_embeddings, d)
        print('Done.')
        epochs = self.num_label_epochs
        batch_size = self.label_batch_size
        print('Pairing labels and features for training surrogate task')
        doc_embeddings = doc_embeddings.cpu()
        count = sum([len(i) for i in d])
        X = torch.zeros((count,), dtype=torch.int64)
        y = torch.zeros((count, 300), dtype=torch.float32)
        print(count)
        count = 0
        for idx, indices in tqdm(enumerate(d), total=len(d)):
            for ind in indices:
                X[count] += ind
                y[count] += doc_embeddings[idx]
                count += 1
        print(count)
        del doc_embeddings
        ...
        optimizer = torch.optim.Adam(self.label_embed_model.fc_layer.parameters(), lr=self.label_embed_lr)
        criterion = DistanceInverseProportionalLoss()
        ...
        for epoch in range(epochs):
            ...
            for i in pbar:
                tmp_X = X[indices[i:i + batch_size]].to(self.label_embed_model.device)
                optimizer.zero_grad()
                loss = criterion(self.label_embed_model(tmp_X),
                                 y[indices[i:i + batch_size]].to(self.label_embed_model.device))
                running_loss += loss.item() * tmp_X.size(0)
                cnt += tmp_X.size(0)
                loss.backward()
                optimizer.step()
                pbar.set_description(
                    "Loss: {:.5f} Avg loss: {:.5f}".format(
                        loss.item(),
                        running_loss / cnt
                    )
                )
        print("Module 1: %s secs" % (time.time() - start_time))


    def _update_shortlist(self, dataset, mode='train', flag=True):
        """
        Get nearest neighbors for the given data and
         update the shortlist in dataset
        ...
        * flag can be used to ignore (useful in case of precomputed shortlist)
        ...
        if flag:
            ...
            start_time = time.time()
            dataset.update_shortlist(*self._predict_shorty(doc_embeddings))
            print("Module 2: %s" % (time.time() - start_time))

    def _predict_shorty(self, embeddings):
        """
        Get nearest neighbors (and sim) for given document embeddings
        ...
        """
        torch.set_grad_enabled(False)
        return self.shorty.query(embeddings, model_dir=self.model_dir)
        ...
        for epoch in range(init_epoch, init_epoch+num_epochs):
            ...
            if epoch % self.retrain_hnsw_after == 0:
                self.logger.info(
                    "Updating shortlist at epoch: {}".format(epoch))
                shorty_start_t = time.time()
                self._train_label_embedding(train_loader.dataset, epoch)
                self._update_shortlist(
                    dataset=train_loader.dataset,
                    mode='train',
                    flag=self.shorty is not None)
                if validation_loader is not None:
                    self._update_shortlist(
                        ...
                        flag=self.shorty is not None)
                shorty_end_t = time.time()
                self.logger.info("ANN time: {0:.2f} sec".format(
                    shorty_end_t - shorty_start_t))
                self.tracking.shortlist_time = self.tracking.shortlist_time \
                    + shorty_end_t - shorty_start_t
                batch_train_start_time = time.time()
            ...
            if validation_loader is not None and epoch % validate_after == 0:
                ...
                val_end_t = time.time()
                _acc = self.evaluate(
                    true_labels=validation_loader.dataset.labels.data,
                    predicted_labels=predicted_labels,
                    train_labels=train_loader.dataset.labels.data
                )
                self.tracking.validation_time = self.tracking.validation_time \
                    + val_end_t - val_start_t
                self.tracking.mean_val_loss.append(val_avg_loss)
                self.tracking.val_precision.append(_acc['ens'][0])
                self.tracking.val_ndcg.append(_acc['ens'][1])
                self.tracking.val_psprecision.append(_acc['ens'][2])
                self.tracking.val_psndcg.append(_acc['ens'][3])
                self.logger.info("Model saved after epoch: {}".format(epoch))
                self.save_checkpoint(model_dir, epoch+1)
                self.tracking.last_saved_epoch = epoch
                _res = self._format_acc(_acc)
                self.logger.info(
                    "{:s}\n Loss: {:.6f}, Time: {:.2f} sec".format(
                        _res, val_avg_loss, val_end_t-val_start_t))
            self.tracking.last_epoch += 1
    ...
    def fit(self, data_dir, model_dir, result_dir, dataset, learning_rate,
            ...
            normalize_labels=False, validate=False, beta=0.2,
            use_intermediate_for_shorty=True, shortlist_method='static',
            validate_after=5, surrogate_mapping=None, label_mapping=None, feature_type='sparse',
        ...
        * Also prints train time and model size
        ...
        surrogate_mapping: str, optional, default=None
            Re-map clusters as per given mapping
            e.g. when labels are clustered
        label_mapping: str, optional, default=None
        feature_type: str, optional, default='sparse'
            sparse or dense features
        trn_pretrained_shortlist: csr_matrix or None, default=None
            Shortlist for train dataset
    ...
    @property
    def model_size(self):
        s = self.net.model_size + self.label_embed_model.model_size
        if self.shorty is not None:
            return s + self.shorty.model_size
        return s
