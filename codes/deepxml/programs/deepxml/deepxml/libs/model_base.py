...
class ModelBase(object):
    ...
    def __init__(self, params, net, criterion, optimizer, *args, **kwargs):
        ...
        self.eval_A = params.A
        self.eval_B = params.B
    ...
    def _create_dataset(self, data_dir, fname_features, fname_labels=None,
                        ...
                        pretrained_shortlist=None, label_mapping=None):
        """
    ...
    def _format_acc(self, acc):
        ...
        if isinstance(acc, dict):
            ...
            _res += "\nnDCG@k: "
            for key, val in acc.items():
                _val = ','.join(map(lambda x: '%0.2f' % (x*100), val[1]))
                _res += "({}): {} ".format(key, _val)
            _res += "\nPSP@k: "
            for key, val in acc.items():
                if len(val) >= 3:
                    _val = ','.join(map(lambda x: '%0.2f' % (x*100), val[2]))
                    _res += "({}): {} ".format(key, _val)
            _res += "\nPSnDCG@k: "
            for key, val in acc.items():
                if len(val) >= 4:
                    _val = ','.join(map(lambda x: '%0.2f' % (x*100), val[3]))
                    _res += "({}): {} ".format(key, _val)
        else:
    ...
    def _evaluate(self, true_labels, predicted_labels, train_labels=None):
        if train_labels is None:
            inv_psp = None
        else:
            inv_psp = xc_metrics.compute_inv_propesity(train_labels, self.eval_A, self.eval_B)

        acc = xc_metrics.Metrics(true_labels=true_labels, inv_psp=inv_psp)
        acc = acc.eval(predicted_labels.tocsr(), 5)
        return acc

    def evaluate(self, true_labels, predicted_labels, train_labels=None):
        """
        ...
        train_labels: csr_matrix or dict
            train labels (to compute prop)
        ...
        if issparse(predicted_labels):
            acc = self._evaluate(true_labels, predicted_labels, train_labels)
        else:  # Multiple set of predictions
            acc = {}
            for key, val in predicted_labels.items():
                acc[key] = self._evaluate(true_labels, val, train_labels)
        return acc
