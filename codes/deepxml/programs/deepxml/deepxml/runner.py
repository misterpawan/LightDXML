...
import tools.autoencode as autoencode
...
def create_autoencode(data_dir, g_config, seed):
    dataset = g_config['dataset']
    arch = g_config['arch']
    label_embed_dims = g_config["label_embed_dims"]
    tmp_model_dir = os.path.join(
        data_dir, dataset, f'deepxml.{arch}', f"{label_embed_dims}.{seed}")
    data_dir = os.path.join(data_dir, dataset)
    try:
        os.makedirs(tmp_model_dir, exist_ok=False)
        autoencode.run(
        ...
        data_stats = json.load(
            open(os.path.join(tmp_model_dir, "data_stats.json")))
        mapping = os.path.join(tmp_model_dir, 'label_mapping.txt')
    return data_stats, mapping
...
def run_deepxml(work_dir, version, seed, config):
    ...
    new_t = time.time()
    print("Surrogate Task Time = ", new_t-cur_t)
    cur_t = new_t
    ...
    new_t = time.time()
    print("Intermediate Representation Training Time = ", new_t-cur_t)
    cur_t = new_t
    ...
    new_t = time.time()
    print("Final representation and Learning Task Time = ", new_t-cur_t)
    cur_t = new_t
    ...
    if use_reranker:
        ...
        avg_prediction_time += _pred_time
        new_t = time.time()
        print("Re-ranker Task Time = ", new_t-cur_t)
        cur_t = new_t
        shutil.copy(
...
def run_deepxml_deepwalk(work_dir, version, seed, config):
    g_config = config['global']
    dataset = g_config['dataset']
    arch = g_config['arch']
    ...
    data_dir = os.path.join(work_dir, 'data')
    result_dir = os.path.join(
        work_dir, 'results', 'DeepXML-RandomWalk', arch, dataset, f'v_{version}')
    model_dir = os.path.join(
        work_dir, 'models', 'DeepXML-RandomWalk', arch, dataset, f'v_{version}')
    filter_fname = os.path.join(data_dir, dataset, 'filter_labels_test.txt')
    if not os.path.isfile(filter_fname):
        filter_fname = None
    ...
    new_t = time.time()
    print("Extreme Classifier Training Time = ", new_t-cur_t)
    cur_t = new_t
...
def run_deepxml_autoencode(work_dir, version, seed, config):
    g_config = config['global']
    dataset = g_config['dataset']
    arch = g_config['arch']
    ...
    data_dir = os.path.join(work_dir, 'data')
    result_dir = os.path.join(
        work_dir, 'results', 'DeepXML-Autoencode', arch, dataset, f'v_{version}')
    model_dir = os.path.join(
        work_dir, 'models', 'DeepXML-Autoencode', arch, dataset, f'v_{version}')
    filter_fname = os.path.join(data_dir, dataset, 'filter_labels_test.txt')
    if not os.path.isfile(filter_fname):
        filter_fname = None
    ...
    data_stats, label_mapping = create_autoencode(
        data_dir, g_config, seed)
...
def run_one(work_dir, model_type, version, seed, config):
    ...
    elif model_type == "DeepXML-DeepWalk":
        return run_deepxml_deepwalk(work_dir, version, seed, config)
    elif model_type == "DeepXML-Autoencoder":
        return run_deepxml_autoencode(work_dir, version, seed, config)
    else:
